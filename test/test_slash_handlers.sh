...
_SLASH_START=$(grep -n '^_slash_plan()' "$LMGPT" | head -1 | cut -d: -f1)
_SLASH_END=$(grep -n '^_rules_reminder_inject()' "$LMGPT" | head -1 | cut -d: -f1)
_TAIL=$(tail -n +"$_SLASH_END" "$LMGPT" | grep -n '^}' | head -1 | cut -d: -f1)
_SLASH_END=$((_SLASH_END + _TAIL - 1))
sed -n "${_SLASH_START},${_SLASH_END}p" "$LMGPT" > /tmp/lmgpt_slash_funcs.sh
...
cat > /tmp/lmgpt_slash_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_slash_funcs.sh
...
MESSAGES='[]'
TODOS='[]'
LMGPT_HISTORY_FILE="/tmp/test_history.json"
LMGPT_MODEL="test-model"
LMGPT_API_URL="http://test/api"
LMGPT_PROJECT_DIR="/tmp/test_proj"
LMGPT_SAFE_MODE=0
...
_stream_reset; LMGPT_MAIN_PROFILE="custom"
_slash_model "/model default"
[[ -z "${LMGPT_MAIN_PROFILE:-}" ]] && _green "T8: /model default switch" \
...
[[ "$LMGPT_SAFE_MODE" == "1" ]] && _green "T20: /safe on → 1" \
    || _red "T20: /safe on" "1" "$LMGPT_SAFE_MODE"

# T21: /safe off
_slash_safe "/safe off" >/dev/null 2>&1
[[ "$LMGPT_SAFE_MODE" == "0" ]] && _green "T21: /safe off → 0" \
    || _red "T21: /safe off" "0" "$LMGPT_SAFE_MODE"

# T22: /safe toggle
LMGPT_SAFE_MODE=0
_slash_safe "/safe toggle" >/dev/null 2>&1
[[ "$LMGPT_SAFE_MODE" == "1" ]] && _green "T22: /safe toggle 0→1" \
    || _red "T22: /safe toggle" "1" "$LMGPT_SAFE_MODE"

# T23: /safe toggle back
_slash_safe "/safe toggle" >/dev/null 2>&1
[[ "$LMGPT_SAFE_MODE" == "0" ]] && _green "T23: /safe toggle 1→0" \
    || _red "T23: /safe toggle" "0" "$LMGPT_SAFE_MODE"
...
done < <(bash /tmp/lmgpt_slash_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_slash_funcs.sh /tmp/lmgpt_slash_test.sh
