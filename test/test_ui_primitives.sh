...
_UI_START=$(grep -n '^ui_spinner()' "$LMGPT" | head -1 | cut -d: -f1)
_UI_END=$(grep -n '^_hook_render_template()' "$LMGPT" | head -1 | cut -d: -f1)
sed -n "${_UI_START},$((_UI_END - 1))p" "$LMGPT" > /tmp/lmgpt_ui_funcs.sh
...
cat > /tmp/lmgpt_ui_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_ui_funcs.sh
...
done < <(bash /tmp/lmgpt_ui_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_ui_funcs.sh /tmp/lmgpt_ui_test.sh
