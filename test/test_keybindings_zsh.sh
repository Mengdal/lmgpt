...
# Generate the zsh keybinding file from lmgpt
_zsh_kbfile="/tmp/lmgpt_test_keybindings.zsh"
_zsh_kbfile2="$HOME/.lmgpt/keybindings.zsh"

# Use installed file if available (from a prior --install), otherwise generate
if [[ -f "$_zsh_kbfile2" ]]; then
    _kb="$_zsh_kbfile2"
else
    # Extract the zsh heredoc from lmgpt for testing
    _kb="$_zsh_kbfile"
    sed -n '/^_gen_keybindings_zsh()/,/^}/p' "$SCRIPT_DIR/../lmgpt" 2>/dev/null | \
        sed -n '/cat >.*ZBEOF/,/^ZBEOF/p' | sed '1s/.*<< .ZBEOF//' | \
        head -n -1 > "$_zsh_kbfile" 2>/dev/null
    if [[ ! -s "$_zsh_kbfile" ]]; then
        _skip "Z1: could not extract zsh template — run lmgpt --install first"
