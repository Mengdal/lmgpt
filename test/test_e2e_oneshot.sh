...
out=$(echo "reply OK" | timeout 60 bash "$LMGPT" --oneshot --stream 2>/dev/null)
...
out=$(echo "reply: hello" | timeout 60 bash "$LMGPT" --oneshot --stream 2>/dev/null)
if echo "$out" | grep -q '"type":"done"'; then
...
out=$(echo "" | timeout 10 bash "$LMGPT" --oneshot --stream 2>/dev/null)
if [[ $? -eq 0 ]]; then
