...
sed -n '517,750p' "$LMGPT" > /tmp/lmgpt_log_funcs.sh
...
cat > /tmp/lmgpt_log_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_log_funcs.sh
...
[[ -f "$LOG_DIR/lmgpt.log" ]] && _green "T2: log creates file" || _red "T2: log file" "exists" "missing"

# T3: log file contains message
grep -q "test message" "$LOG_DIR/lmgpt.log" 2>/dev/null && _green "T3: message in log" \
    || _red "T3: message" "test message" "$(cat "$LOG_DIR/lmgpt.log")"

# T4: log includes level
grep -q "INFO" "$LOG_DIR/lmgpt.log" 2>/dev/null && _green "T4: level in log" \
    || _red "T4: level" "INFO" "$(cat "$LOG_DIR/lmgpt.log")"

# T5: log includes caller info (function:line)
log "INFO:" "caller test"
grep -qE "\[.*:[0-9]+\]" "$LOG_DIR/lmgpt.log" 2>/dev/null && _green "T5: caller info present" \
    || _red "T5: caller" "[func:line]" "$(cat "$LOG_DIR/lmgpt.log")"
...
grep -q "info msg should appear" "$LOG_DIR/lmgpt.log" && _green "T6: INFO logged at INFO threshold" \
    || _red "T6: INFO" "info msg" "$(cat "$LOG_DIR/lmgpt.log")"

# T7: WARN level
> "$LOG_DIR/lmgpt.log"
log "WARN:" "warning msg"
grep -q "WARN" "$LOG_DIR/lmgpt.log" && _green "T7: WARN logged" \
    || _red "T7: WARN" "WARN" "$(cat "$LOG_DIR/lmgpt.log")"

# T8: ERROR level
> "$LOG_DIR/lmgpt.log"
log "ERROR:" "error occurred"
grep -q "ERROR" "$LOG_DIR/lmgpt.log" && _green "T8: ERROR logged" \
    || _red "T8: ERROR" "ERROR" "$(cat "$LOG_DIR/lmgpt.log")"
...
_log_rotate "$LOG_DIR" "lmgpt.log" >/dev/null 2>&1
...
_lines=$(grep -c "stress test message" "$LOG_DIR/lmgpt.log" 2>/dev/null || echo 0)
...
done < <(bash /tmp/lmgpt_log_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_log_funcs.sh /tmp/lmgpt_log_test.sh
