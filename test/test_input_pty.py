#!/usr/bin/env python3
"""PTY-based E2E test driver for lmgpt input system.

Usage: python3 test/test_input_pty.py [--test NAME] [--verbose]

Creates a pseudo-terminal, spawns lmgpt in interactive mode, sends
...
def run_pty_test(test_name, inputs, checks, rows=24, cols=80, timeout=8,
    ...
    """Run lmgpt in a PTY, send inputs, run checks.
    ...
    pid, fd = pty.fork()
    if pid == 0:
        # Child: configure environment and exec lmgpt
        os.environ['TERM'] = 'xterm-256color'
        os.environ['COLUMNS'] = str(cols)
        os.environ['LINES'] = str(rows)
        os.environ['LMGPT_LOG_LEVEL'] = 'ERROR'
        # Suppress logging to stderr during tests
        os.environ['LMGPT_LOG_STDERR'] = '0'
        # Dummy API key — won't be used since we kill before API call
        if 'LMGPT_API_KEY' not in os.environ:
            os.environ['LMGPT_API_KEY'] = 'sk-test-dummy-key'

        if mode == 'oneshot':
            # Oneshot reads from stdin
            os.execvp('bash', ['bash', '-c', 'exec lmgpt --oneshot --stream'])
        else:
            os.execvp('bash', ['bash', '-c', 'exec lmgpt'])

    # Parent: drive the PTY
    try:
        set_pty_size(fd, rows, cols)

        # Wait for lmgpt prompt to appear (signals _input_readline is active
...
def test_cjk_backspace():
    ...
    run_pty_test(
        ...
        start_delay=2.5,       # lmgpt init takes ~2s (banner, config, agents)
        post_delay=0.5,
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='lmgpt input system PTY tests')
