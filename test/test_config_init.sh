...
_START=$(grep -n '^_detect_termux()' "$LMGPT" | head -1 | cut -d: -f1)
_END=$(grep -n '^_reload_skills_if_stale()' "$LMGPT" | head -1 | cut -d: -f1)
sed -n "${_START},$((_END - 1))p" "$LMGPT" > /tmp/lmgpt_config_funcs.sh
...
cat > /tmp/lmgpt_config_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_config_funcs.sh
...
SANDBOX=$(mktemp -d)
LMGPT_PROJECT_DIR="$SANDBOX/proj"

# T1: creates base .lmgpt dir
init_project_dirs
[[ -d "$SANDBOX/proj/.lmgpt" ]] && _green "T1: creates .lmgpt" || _red "T1: dir" "exists" "missing"

# T2: creates agents dir
[[ -d "$SANDBOX/proj/.lmgpt/agents" ]] && _green "T2: agents dir" || _red "T2: agents" "exists" "missing"

# T3: creates skills dir
[[ -d "$SANDBOX/proj/.lmgpt/skills" ]] && _green "T3: skills dir" || _red "T3: skills" "exists" "missing"

# T4: creates comm dir
[[ -d "$SANDBOX/proj/.lmgpt/comm" ]] && _green "T4: comm dir" || _red "T4: comm" "exists" "missing"

# T5: creates hooks dir
[[ -d "$SANDBOX/proj/.lmgpt/hooks" ]] && _green "T5: hooks dir" || _red "T5: hooks" "exists" "missing"

# T6: creates trace dirs
[[ -d "$SANDBOX/proj/.lmgpt/trace/frames" ]] && _green "T6: trace/frames dir" \
    || _red "T6: trace" "exists" "missing"

# ════════════════ T7-T10: _mktemp_file / _mktemp_dir / _mktemp_u ════════════════

# T7: _mktemp_file creates temp file
_f=$(_mktemp_file /tmp/lmgpt_test.XXXXXX)
[[ -f "$_f" ]] && _green "T7: _mktemp_file works" || _red "T7: mktemp_file" "exists" "missing"
rm -f "$_f"

# T8: _mktemp_dir creates dir
_d=$(_mktemp_dir /tmp/lmgpt_testd.XXXXXX)
[[ -d "$_d" ]] && _green "T8: _mktemp_dir works" || _red "T8: mktemp_dir" "exists" "missing"
rmdir "$_d"

# T9: _mktemp_u returns unused name
_u=$(_mktemp_u /tmp/lmgpt_testu.XXXXXX)
...
for i in $(seq 1 20); do
    _f=$(_mktemp_file /tmp/lmgpt_stress.XXXXXX)
    rm -f "$_f"
done
...
done < <(bash /tmp/lmgpt_config_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_config_funcs.sh /tmp/lmgpt_config_test.sh
