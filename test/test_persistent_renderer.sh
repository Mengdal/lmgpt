...
_run_group() {
    ...
    bash -c "
        set -e
        LMGPT_TEST_MODE=1 source ./lmgpt >/dev/null 2>&1
        $_code
...
_E_f1=$(_mktemp_u /tmp/lmgpt_e1.XXXXXX)
_E_f2=$(_mktemp_u /tmp/lmgpt_e2.XXXXXX)
_assert "E2: Unique per-turn FIFO names" '[[ "$_E_f1" != "$_E_f2" ]]'
rm -f "$_E_f1" "$_E_f2"

# Per-turn renderer exits on done
_E_fifo=$(_mktemp_u /tmp/lmgpt_ef.XXXXXX); mkfifo "$_E_fifo" 2>/dev/null
...
_H_count=$(grep -c '_stream_kv status_update' ./lmgpt 2>/dev/null || echo 0)
