...
# Run: export LMGPT_API_KEY="sk-..." && bash test/test_hooks_e2e.sh
set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LMGPT="$SCRIPT_DIR/../lmgpt"
PASS=0; FAIL=0
TEST_DIR=$(mktemp -d "/tmp/lmgpt_hook_e2e.XXXXXX")
...
[[ -x "$LMGPT" ]] || { echo "ERROR: lmgpt not found"; exit 1; }
[[ -n "${LMGPT_API_KEY:-}" ]] || { echo "ERROR: LMGPT_API_KEY not set"; exit 1; }
export LMGPT_LOG_LEVEL=ERROR
export LMGPT_LOG_STDERR=0
...
run_oneshot() {
    ...
    printf '%s' "$_prompt" | LMGPT_PROJECT_DIR="$_dir" "$LMGPT" --oneshot 2>/dev/null
}
...
test_post_tool_history() {
    ...
    mkdir -p "$_dir/.lmgpt/hooks"
    echo "# Test" > "$_dir/.lmgpt/LMGPT.md"
    cat > "$_dir/.lmgpt/hooks/99_test.sh" <<'SHEOF'
...
SHEOF
    ...
    local _hist="$_dir/.lmgpt/history.json"
...
test_pre_turn_fires() {
    ...
    cat > "$_dir/.lmgpt/hooks/00_inject.sh" <<'SHEOF'
...
test_dyn_msg_isolation() {
    ...
    mkdir -p "$_dir/.lmgpt/hooks"
    echo "# Test" > "$_dir/.lmgpt/LMGPT.md"
    cat > "$_dir/.lmgpt/hooks/00_pre.md" <<'MDEOF'
