...
_run_oneshot() {
    ...
    local tmpdir; tmpdir=$(mktemp -d /tmp/lmgpt_reg.XXXXXX)
    echo "$prompt" | LMGPT_PROJECT_DIR="$tmpdir" timeout "$timeout_s" bash "$LMGPT" --oneshot --stream 2>/dev/null
    local rc=$?
    rm -rf "$tmpdir"
    return $rc
}
...
out=$(echo "/exit" | timeout 5 bash "$LMGPT" --oneshot 2>/dev/null)
rc=$?
if [[ $rc -eq 0 ]]; then
...
out=$(printf '/exit\n' | timeout 5 bash "$LMGPT" --oneshot --stream 2>/dev/null)
rc=$?
if [[ $rc -eq 0 ]]; then
...
out=$(printf '/help\n/status\n/exit\n' | timeout 10 bash "$LMGPT" --oneshot --stream 2>/dev/null | head -5)
rc=$?
...
out=$(echo "" | timeout 5 bash "$LMGPT" --oneshot --stream 2>/dev/null)
rc=$?
if [[ $rc -eq 0 ]]; then
