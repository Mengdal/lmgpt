...
_CACHE_START=$(grep -n '^_comphash()' "$LMGPT" | head -1 | cut -d: -f1)
_CACHE_END=$(grep -n '^_reload_skills_if_stale()' "$LMGPT" | head -1 | cut -d: -f1)
_CACHE_END=$((_CACHE_END - 1))
sed -n "${_CACHE_START},${_CACHE_END}p" "$LMGPT" > /tmp/lmgpt_cache_funcs.sh
...
cat > /tmp/lmgpt_cache_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_cache_funcs.sh
...
declare -A _CC=()
CACHE_DIR=$(mktemp -d)
LMGPT_PROJECT_DIR="$CACHE_DIR"
mkdir -p "$CACHE_DIR/.lmgpt"
MESSAGES='[]'
...
_cc_persist
[[ -f "$CACHE_DIR/.lmgpt/cache_state.json" ]] && _green "T16: persist creates cache_state.json" \
...
_pfile="$CACHE_DIR/.lmgpt/cache_state.json"
if [[ -f "$_pfile" ]]; then
    _content=$(cat "$_pfile")
...
for _i in $(seq 1 10); do
    ...
    _cc_persist
    [[ -f "$CACHE_DIR/.lmgpt/cache_state.json" ]] || { _s_ok=0; break; }
done
...
done < <(bash /tmp/lmgpt_cache_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_cache_funcs.sh /tmp/lmgpt_cache_test.sh
