...
_PROTO_START=$(grep -n '^_reload_skills_if_stale()' "$LMGPT" | head -1 | cut -d: -f1)
_PROTO_END=$(grep -n '^_http_map_exit()' "$LMGPT" | head -1 | cut -d: -f1)
_PROTO_END=$((_PROTO_END - 1))
sed -n "${_PROTO_START},${_PROTO_END}p" "$LMGPT" > /tmp/lmgpt_proto_funcs.sh
...
# ── Write test script ──
cat > /tmp/lmgpt_proto_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_proto_funcs.sh
...
# ════════════════ T1-T6: _lmgpt_md_has_content ════════════════

# T1: LMGPT_MD has real content → true
printf -v LMGPT_MD '# Project: Test\n\nDescription: A test project'
_lmgpt_md_has_content && _green "T1: has content → true" || _red "T1: content" "true" "false"

# T2: LMGPT_MD empty → false
LMGPT_MD=""
_lmgpt_md_has_content && _red "T2: empty → false" "false" "true" || _green "T2: empty → false"

# T3: LMGPT_MD only comments → false
printf -v LMGPT_MD '# just a comment\n# another comment'
_lmgpt_md_has_content && _red "T3: comments only → false" "false" "true" || _green "T3: comments only → false"

# T4: LMGPT_MD unset → false
unset LMGPT_MD
_lmgpt_md_has_content && _red "T4: unset → false" "false" "true" || _green "T4: unset → false"
...
_lmgpt_md_has_content; _rc5=$?
[[ $_rc5 -eq 0 || $_rc5 -eq 1 ]] && _green "T5: whitespace handled (rc=$_rc5)" \
    || _red "T5: whitespace" "0 or 1" "rc=$_rc5"

# T6: LMGPT_MD has real content after a comment line
printf -v LMGPT_MD '# comment\nreal content here'
_lmgpt_md_has_content && _green "T6: mixed → true" || _red "T6: mixed" "true" "false"
LMGPT_MD=""

# ════════════════ T7-T14: _pe_assemble_system ════════════════

# T7: basic output with empty LMGPT_MD
LMGPT_MD=""
...
# T8: LMGPT_MD content replaces identity
printf -v LMGPT_MD '# Custom Identity\nI am a custom agent'
_result=$(_pe_assemble_system 0)
echo "$_result" | grep -q "Custom Identity" && _green "T8: LMGPT_MD in output" \
    || _red "T8: LMGPT_MD" "Custom Identity" "$(echo "$_result" | head -c 200)"

# T9: without LMGPT_MD, uses default identity
LMGPT_MD=""
_result=$(_pe_assemble_system 0)
...
done < <(bash /tmp/lmgpt_proto_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_proto_funcs.sh /tmp/lmgpt_proto_test.sh
