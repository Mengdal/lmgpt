...
_has_key=0
if [[ -n "${LMGPT_API_KEY:-}" ]]; then
    _has_key=1
elif grep -q '"api_key"' "$HOME/.lmgpt/settings.json" 2>/dev/null; then
    _has_key=1
fi
...
_slash_test() {
    ...
    [[ -z "$tmpdir" ]] && tmpdir=$(mktemp -d /tmp/lmgpt_slash.XXXXXX)
    local out rc
    out=$(echo "$cmd" | LMGPT_PROJECT_DIR="$tmpdir" timeout "$to" bash "$LMGPT" --oneshot --stream 2>/dev/null)
    rc=$?
    echo "$out"
    return $rc
}
...
tmpdir=$(mktemp -d /tmp/lmgpt_slash_cl.XXXXXX)
# First add a message, then clear
echo "hello" | LMGPT_PROJECT_DIR="$tmpdir" timeout 60 bash "$LMGPT" --oneshot --stream 2>/dev/null >/dev/null || true
echo "/clear" | LMGPT_PROJECT_DIR="$tmpdir" timeout 10 bash "$LMGPT" --oneshot --stream 2>/dev/null >/dev/null || true
status_out=$(echo "/status" | LMGPT_PROJECT_DIR="$tmpdir" timeout 10 bash "$LMGPT" --oneshot --stream 2>/dev/null)
...
if [[ $_has_key -eq 0 ]]; then
    ...
    out=$(echo "/plan" | timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    rc=$?
    [[ $rc -eq 0 ]] && _pass "S17: /plan shows plan status" \
        || _fail "S17: /plan (rc=$rc)"

    # S18: /plan <text> — should fallthrough to agent (may timeout)
    out=$(echo "/plan test: say OK" | timeout 180 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    rc=$?
    ...
    # S19: /remember
    out=$(echo "/remember Test memory: lmgpt is a bash AI agent" | timeout 60 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    rc=$?
    if [[ $rc -eq 0 ]]; then
    ...
    # S20: /compress
    out=$(echo "/compress" | timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    rc=$?
    [[ $rc -eq 0 ]] && _pass "S20: /compress works" \
        || _fail "S20: /compress (rc=$rc)"

    # S21-S26: TODO commands (need prior TODO state)
    tmpdir=$(mktemp -d /tmp/lmgpt_slash_td.XXXXXX)
    echo "/todo-add Test task one" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null >/dev/null
    out=$(echo "/todo-add Test task two" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S21: /todo-add works" \
        || _fail "S21: /todo-add (rc=$rc)"

    out=$(echo "/todo-start 1" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S22: /todo-start works" \
        || _fail "S22: /todo-start (rc=$rc)"

    out=$(echo "/todo-done 1" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S23: /todo-done works" \
        || _fail "S23: /todo-done (rc=$rc)"

    out=$(echo "/todo-fail 2" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S24: /todo-fail works" \
        || _fail "S24: /todo-fail (rc=$rc)"

    out=$(echo "/todo-edit 2 Updated test task" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S25: /todo-edit works" \
        || _fail "S25: /todo-edit (rc=$rc)"

    out=$(echo "/todo-delete 2" | LMGPT_PROJECT_DIR="$tmpdir" timeout 30 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S26: /todo-delete works" \
        || _fail "S26: /todo-delete (rc=$rc)"
    rm -rf "$tmpdir"

    # S27: /skill and /skill-off
    out=$(echo "/skill test_skill 2>/dev/null" | timeout 15 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S27: /skill works (shows error if skill missing)" \
        || _fail "S27: /skill (rc=$rc)"

    out=$(echo "/skill-off test_skill" | timeout 15 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S28: /skill-off works" \
        || _fail "S28: /skill-off (rc=$rc)"

    # S29: /task-cancel (should error on invalid ID, not crash)
    out=$(echo "/task-cancel 99999" | timeout 15 bash "$LMGPT" --oneshot --stream 2>/dev/null)
    [[ $rc -eq 0 ]] && _pass "S29: /task-cancel handles invalid ID" \
        || _fail "S29: /task-cancel (rc=$rc)"

    # S30: /mcp list
    out=$(echo "/mcp list" | timeout 15 bash "$LMGPT" --oneshot --stream 2>/dev/null)
