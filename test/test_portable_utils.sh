...
# Extract a single function definition from lmgpt and run it in isolation.
...
_run_func() {
    ...
    local def; def=$(sed -n "/^${func}()/,/^}/p" "$LMGPT" 2>/dev/null)
    if [[ -z "$def" ]]; then
        echo "ERROR: could not extract $func from lmgpt" >&2
        return 1
    fi
...
mt=$(_run_func _file_mtime "" "$LMGPT")
...
h=$(printf 'lmgpt' | _run_func _cc_hash "lmgpt")
if [[ "${#h}" -eq 64 ]] && [[ "$h" =~ ^[0-9a-f]+$ ]]; then
    _pass "'lmgpt' → valid 64-char hex: ${h:0:16}..."
else
...
lockf2="/tmp/lmgpt_test_lock_nb_$$"
rm -rf "${lockf2}.lock" 2>/dev/null || true
nb_out=$(bash -c "
$(sed -n '/^_lock_acquire()/,/^}/p' "$LMGPT")
$(sed -n '/^_lock_acquire_nb()/,/^}/p' "$LMGPT")
$(sed -n '/^_lock_release()/,/^}/p' "$LMGPT")
...
if grep -q "sed.*\\\\033" "$LMGPT"; then
    ...
    if grep -q 'x1b' <(grep 'sed.*s/' "$LMGPT" 2>/dev/null); then
...
tmpf=$(mktemp /tmp/lmgpt_test_printf.XXXXXX)
printf '' > "$tmpf"
