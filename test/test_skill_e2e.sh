...
#
# Strategy: source the full lmgpt in a restricted subshell that
# sets HOME to a temp dir (so ~/.lmgpt/config doesn't exist and
# lmgpt exits early on full init). But we only source the skill
...
[[ -f "$LMGPT" ]] || { echo "ERROR: lmgpt not found at $BASHAGT"; exit 1; }

TEST_HOME="$(mktemp -d /tmp/lmgpt_skill_test_XXXXXX)"
trap 'rm -rf "$TEST_HOME"' EXIT

# Create test directories
mkdir -p "$TEST_HOME/.lmgpt/skills"
mkdir -p "$TEST_HOME/project/.lmgpt/skills"
...
SEC7B_START=$(grep -n 'SECTION 7b:' "$LMGPT" | head -1 | cut -d: -f1)
SEC7C_START=$(grep -n 'SECTION 7c:' "$LMGPT" | head -1 | cut -d: -f1)
...
# Build a standalone skill library
{
    ...
    sed -n "${SEC7B_START},$((SEC7C_START - 1))p" "$LMGPT"
} > "$SKILL_LIB"

# Also extract build_sys_static and its cache dependencies
BUILD_START=$(grep -n '^build_sys_static()' "$LMGPT" | head -1 | cut -d: -f1)
BUILD_END=$(grep -n '^build_dyn_context_msg()' "$LMGPT" | head -1 | cut -d: -f1)
BUILD_END=$((BUILD_END - 1))

# _cc_hash function
CC_HASH_START=$(grep -n '^_cc_hash()' "$LMGPT" | head -1 | cut -d: -f1)
CC_HASH_END=$(tail -n +$((CC_HASH_START + 1)) "$LMGPT" | grep -n '^}' | head -1 | cut -d: -f1)
CC_HASH_END=$((CC_HASH_START + CC_HASH_END))

# _cc_get and _cc_put functions
CC_GET_START=$(grep -n '^_cc_get()' "$LMGPT" | head -1 | cut -d: -f1)
CC_GET_END=$(tail -n +$((CC_GET_START + 1)) "$LMGPT" | grep -n '^}' | head -1 | cut -d: -f1)
CC_GET_END=$((CC_GET_START + CC_GET_END))

CC_PUT_START=$(grep -n '^_cc_put()' "$LMGPT" | head -1 | cut -d: -f1)
CC_PUT_END=$(tail -n +$((CC_PUT_START + 1)) "$LMGPT" | grep -n '^}' | head -1 | cut -d: -f1)
...
FULL_LIB="$TEST_HOME/full_lib.sh"
{
    ...
    echo 'CACHE_TABLE=()'
    # _cc_hash
    sed -n "${CC_HASH_START},${CC_HASH_END}p" "$LMGPT"
    # _cc_get
    sed -n "${CC_GET_START},${CC_GET_END}p" "$LMGPT"
    # _cc_put
    sed -n "${CC_PUT_START},${CC_PUT_END}p" "$LMGPT"
    # Skill section
    sed -n "${SEC7B_START},$((SEC7C_START - 1))p" "$LMGPT"
    # build_sys_static
    sed -n "${BUILD_START},${BUILD_END}p" "$LMGPT"
} > "$FULL_LIB"
...
mkdir -p "$TEST_HOME/.lmgpt/skills/my-skill"
cat > "$TEST_HOME/.lmgpt/skills/my-skill/skill.md" << 'EOF'
...
mkdir -p "$TEST_HOME/.lmgpt/skills/multi-skill"
cat > "$TEST_HOME/.lmgpt/skills/multi-skill/skill.md" << 'EOF'
{
  "name": "multi-skill",
...
mkdir -p "$TEST_HOME/.lmgpt/skills/noname"
cat > "$TEST_HOME/.lmgpt/skills/noname/skill.md" << 'EOF'
{"description": "No name field here"}
This skill has no name in JSON, dir name = noname.
EOF

# Skill 4: no JSON at all — should be rejected
mkdir -p "$TEST_HOME/.lmgpt/skills/nojson"
cat > "$TEST_HOME/.lmgpt/skills/nojson/skill.md" << 'EOF'
This file has no JSON frontmatter at all.
Just plain text.
EOF

# Skill 5: empty body
mkdir -p "$TEST_HOME/.lmgpt/skills/empty-body"
cat > "$TEST_HOME/.lmgpt/skills/empty-body/skill.md" << 'EOF'
{"name": "empty-body", "description": "Body is empty"}
EOF

# Project skill (different name from system skills)
mkdir -p "$TEST_HOME/project/.lmgpt/skills/proj-skill"
cat > "$TEST_HOME/project/.lmgpt/skills/proj-skill/skill.md" << 'EOF'
...
mkdir -p "$TEST_HOME/project/.lmgpt/skills/my-skill"
cat > "$TEST_HOME/project/.lmgpt/skills/my-skill/skill.md" << 'EOF'
...
result=$(_run_test "parse single" '
    parse_skill_file "$HOME/.lmgpt/skills/my-skill/skill.md" "my-skill"
...
result=$(_run_test "parse multi" '
    parse_skill_file "$HOME/.lmgpt/skills/multi-skill/skill.md" "multi-skill"
...
result=$(_run_test "no name, no opt" '
    if parse_skill_file "$HOME/.lmgpt/skills/noname/skill.md"; then
        echo "RETURNED_OK"
    else
        echo "RETURNED_FAIL"
    fi
...
result=$(_run_test "no name, with opt" '
    if parse_skill_file "$HOME/.lmgpt/skills/noname/skill.md" "noname"; then
        echo "RETURNED_OK"
    else
        echo "RETURNED_FAIL"
    fi
...
result=$(_run_test "no JSON, no opt" '
    if parse_skill_file "$HOME/.lmgpt/skills/nojson/skill.md"; then
        echo "RETURNED_OK"
    else
        echo "RETURNED_FAIL"
    fi
...
result=$(_run_test "no JSON, with opt" '
    if parse_skill_file "$HOME/.lmgpt/skills/nojson/skill.md" "nojson"; then
...
result=$(_run_test "empty body" '
    parse_skill_file "$HOME/.lmgpt/skills/empty-body/skill.md" "empty-body"
    printf "body_len=%d\n" "${#SKILLS[empty-body]}"
')
...
rm -rf "$TEST_HOME/.lmgpt/skills/multi-skill"
...
result=$(_run_full_test "injection" '
    ...
    # Set required vars
    LMGPT_MD=""
    LMGPT_SYSTEM_PROMPT="You are a test assistant."
    AGENT_DESCRIPTIONS="agent1: does things"
    raw="${LMGPT_MD:+$LMGPT_MD\n\n}${LMGPT_SYSTEM_PROMPT}
...
mkdir -p "$TEST_HOME/.lmgpt/skills/empty-dir"
...
mkdir -p "$TEST_HOME/.lmgpt/skills/dir-name-wins"
cat > "$TEST_HOME/.lmgpt/skills/dir-name-wins/skill.md" << 'EOF'
