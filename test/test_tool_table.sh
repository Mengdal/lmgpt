...
# ── Extract required functions from lmgpt ──
...
sed -n '/^_is_wide_codepoint()/,/^}/p' "$LMGPT" > "$EXTRACT_DIR/funcs.sh"
echo >> "$EXTRACT_DIR/funcs.sh"
sed -n '/^_is_zero_width_codepoint()/,/^}/p' "$LMGPT" >> "$EXTRACT_DIR/funcs.sh"
echo >> "$EXTRACT_DIR/funcs.sh"
sed -n '/^_str_display_width()/,/^}/p' "$LMGPT" >> "$EXTRACT_DIR/funcs.sh"
echo >> "$EXTRACT_DIR/funcs.sh"

# Extract _strip_ansi_sgr
sed -n '/^_strip_ansi_sgr()/,/^}/p' "$LMGPT" >> "$EXTRACT_DIR/funcs.sh"
echo >> "$EXTRACT_DIR/funcs.sh"

# Extract tool_table
sed -n '/^tool_table()/,/^}/p' "$LMGPT" >> "$EXTRACT_DIR/funcs.sh"
