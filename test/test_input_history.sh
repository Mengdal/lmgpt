...
_HIST_START=$(grep -n '^_in_hist_load()' "$LMGPT" | head -1 | cut -d: -f1)
_HIST_END=$(grep -n '^_input_readline()' "$LMGPT" | head -1 | cut -d: -f1)
_HIST_END=$((_HIST_END - 1))
sed -n "${_HIST_START},${_HIST_END}p" "$LMGPT" > /tmp/lmgpt_hist_funcs.sh
...
cat > /tmp/lmgpt_hist_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_hist_funcs.sh
...
done < <(bash /tmp/lmgpt_hist_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_hist_funcs.sh /tmp/lmgpt_hist_test.sh
