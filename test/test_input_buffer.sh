...
_RENDER_START=$(grep -n '^# ── Unicode display-width calculator' "$LMGPT" | head -1 | cut -d: -f1)
_RENDER_END=$(grep -n '^_input_render()' "$LMGPT" | head -1 | cut -d: -f1)
_RENDER_TAIL=$(tail -n +"$_RENDER_END" "$LMGPT" | grep -n '^}' | head -1 | cut -d: -f1)
_RENDER_END=$((_RENDER_END + _RENDER_TAIL - 1))
sed -n "${_RENDER_START},${_RENDER_END}p" "$LMGPT" > /tmp/lmgpt_render_funcs.sh
...
cat > /tmp/lmgpt_buf_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_render_funcs.sh
...
done < <(bash /tmp/lmgpt_buf_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_render_funcs.sh /tmp/lmgpt_buf_test.sh
