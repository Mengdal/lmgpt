#!/usr/bin/env bash
# test_integration.sh — End-to-end integration tests for lmgpt
# Run: bash test/test_integration.sh
# REQUIRES valid API key in ~/.lmgpt/settings.json
...
[[ -f "$LMGPT" ]] || { echo "ERROR: lmgpt not found at $LMGPT"; exit 1; }

# Check API key is configured
if ! grep -q '"api_key"' "$HOME/.lmgpt/settings.json" 2>/dev/null; then
    echo "SKIP: No API key configured in ~/.lmgpt/settings.json"
...
_test_oneshot() {
    ...
    out=$(timeout "$timeout_sec" bash -c "
        echo '$prompt' | bash '$LMGPT' --oneshot --stream 2>/dev/null
