...
_HTTP_START=$(grep -n '^_http_map_exit()' "$LMGPT" | head -1 | cut -d: -f1)
_AGENT_START=$(grep -n '^parse_agent_file()' "$LMGPT" | head -1 | cut -d: -f1)
sed -n "${_HTTP_START},$((_AGENT_START - 1))p" "$LMGPT" > /tmp/lmgpt_http_funcs.sh
...
# ── Write test script ──
cat > /tmp/lmgpt_http_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_http_funcs.sh
...
LMGPT_CONNECT_TIMEOUT=5
LMGPT_PROXY_URL=""
LMGPT_PROXY_USER=""
LMGPT_PROXY_PASS=""
LMGPT_PROXY_NOPROXY="localhost,127.0.0.1,::1"

_SPY_FILE="/tmp/lmgpt_spy_$$.txt"
...
grep -q "BODY:@/tmp/lmgpt_body" "$_SPY_FILE" && _green "T25: POST + body" || _red "T25: POST+body" "BODY:@..." "$(tr '\n' ' ' < $_SPY_FILE)"
...
_spy_reset; LMGPT_PROXY_URL="http://proxy:8080"
...
done < <(bash /tmp/lmgpt_http_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_http_funcs.sh /tmp/lmgpt_http_test.sh /tmp/test_http_out /tmp/test_http_stress /tmp/lmgpt_spy_*.txt
