#!/usr/bin/env bash
# test_trace.sh — Unit + stress tests for lmgpt trace system
...
PASS=0; FAIL=0
TEST_DIR=$(mktemp -d "/tmp/lmgpt_trace_test.XXXXXX")
...
# ── Setup: extract trace functions from lmgpt ──
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LMGPT="$SCRIPT_DIR/../lmgpt"
TRACE_START=$(grep -n "^# SECTION 7f: Trace System" "$LMGPT" | cut -d: -f1)
TRACE_END=$(grep -n "^# SECTION 8: Tool Definitions" "$LMGPT" | cut -d: -f1)
TRACE_END=$((TRACE_END - 1))
sed -n "${TRACE_START},${TRACE_END}p" "$LMGPT" > "$TEST_DIR/trace_funcs.sh"
...
export LMGPT_TRACE_ENABLED=1
export AGENT_SELF_NAME="main"
export LMGPT_TRACE_DESC="test"
export LMGPT_TRACE_TOOL="edit_file"
LMGPT_PROJECT_DIR="$TEST_DIR/proj"
mkdir -p "$LMGPT_PROJECT_DIR"
...
# Single record
mkdir -p "$LMGPT_PROJECT_DIR/.lmgpt"
echo '{"k":"v1"}' > "$LMGPT_PROJECT_DIR/test.json"
old='{"k":"v1"}'; new='{"k":"v2"}'
trace_record "$LMGPT_PROJECT_DIR/test.json" "$old" "$new" \
...
trace_record "$LMGPT_PROJECT_DIR/newfile.txt" "" "new content" \
...
trace_record "$LMGPT_PROJECT_DIR/test.json" "$new" '{"k":"v3"}' \
...
_trace_set_head 1
echo '{"k":"v2"}' > "$LMGPT_PROJECT_DIR/test.json"
...
_restored=$(cat "$LMGPT_PROJECT_DIR/test.json")
...
echo '{"count":0}' > "$LMGPT_PROJECT_DIR/stress.json"
_stress_start=$(date +%s%3N 2>/dev/null || echo 0)
for i in $(seq 1 100); do
    old="{\"count\":$((i-1))}"
    new="{\"count\":$i}"
    trace_record "$LMGPT_PROJECT_DIR/stress.json" "$old" "$new" \
...
echo '{"count":100}' > "$LMGPT_PROJECT_DIR/stress.json"
...
_restored=$(cat "$LMGPT_PROJECT_DIR/stress.json")
[[ "$_restored" == '{"count":50}' ]] && _pass "undo 50: content restored to count=50" || _fail "undo 50: got $_restored"

# Undo remaining 50
echo '{"count":50}' > "$LMGPT_PROJECT_DIR/stress.json"
trace_undo 50 force 2>/dev/null
_restored2=$(cat "$LMGPT_PROJECT_DIR/stress.json")
...
echo '{"x":1}' > "$LMGPT_PROJECT_DIR/verify.json"
trace_record "$LMGPT_PROJECT_DIR/verify.json" '{"x":1}' '{"x":2}' \
    "$(jq -nc --arg a "main" --arg t "edit_file" --arg d "v1" --argjson n 1 '{agent:$a,tool:$t,turn:$n,desc:$d}')"
trace_record "$LMGPT_PROJECT_DIR/verify.json" '{"x":2}' '{"x":3}' \
...
echo '{"x":999}' > "$LMGPT_PROJECT_DIR/verify.json"
...
trace_undo 1 force 2>/dev/null
_restored=$(cat "$LMGPT_PROJECT_DIR/verify.json")
...
printf '%s' "$_special_old" > "$LMGPT_PROJECT_DIR/special.json"
trace_record "$LMGPT_PROJECT_DIR/special.json" "$_special_old" "$_special_new" \
...
# Undo
printf '%s' "$_special_new" > "$LMGPT_PROJECT_DIR/special.json"
trace_undo 1 force 2>/dev/null
_restored=$(cat "$LMGPT_PROJECT_DIR/special.json")
...
echo 'a1' > "$LMGPT_PROJECT_DIR/a.txt"
echo 'b1' > "$LMGPT_PROJECT_DIR/b.txt"
echo 'c1' > "$LMGPT_PROJECT_DIR/c.txt"

# Record each file separately (simulating one turn with 3 edits)
trace_record "$LMGPT_PROJECT_DIR/a.txt" 'a1' 'a2' \
    "$(jq -nc --arg a "main" --arg t "edit_file" --arg d "edit a,b,c" --argjson n 1 '{agent:$a,tool:$t,turn:$n,desc:$d}')"
trace_record "$LMGPT_PROJECT_DIR/b.txt" 'b1' 'b2' \
    "$(jq -nc --arg a "main" --arg t "edit_file" --arg d "edit a,b,c" --argjson n 1 '{agent:$a,tool:$t,turn:$n,desc:$d}')"
trace_record "$LMGPT_PROJECT_DIR/c.txt" 'c1' 'c2' \
    "$(jq -nc --arg a "main" --arg t "edit_file" --arg d "edit a,b,c" --argjson n 1 '{agent:$a,tool:$t,turn:$n,desc:$d}')"
...
# Undo all 3
echo 'a2' > "$LMGPT_PROJECT_DIR/a.txt"
echo 'b2' > "$LMGPT_PROJECT_DIR/b.txt"
echo 'c2' > "$LMGPT_PROJECT_DIR/c.txt"
trace_undo 3 force 2>/dev/null

[[ "$(cat "$LMGPT_PROJECT_DIR/a.txt")" == 'a1' ]] && _pass "multi-undo: a.txt restored" || _fail "multi-undo a.txt: $(cat "$LMGPT_PROJECT_DIR/a.txt")"
[[ "$(cat "$LMGPT_PROJECT_DIR/b.txt")" == 'b1' ]] && _pass "multi-undo: b.txt restored" || _fail "multi-undo b.txt"
[[ "$(cat "$LMGPT_PROJECT_DIR/c.txt")" == 'c1' ]] && _pass "multi-undo: c.txt restored" || _fail "multi-undo c.txt"
...
# New file with empty content
    printf "" > "$LMGPT_PROJECT_DIR/empty.txt"
trace_record "$LMGPT_PROJECT_DIR/empty.txt" "" "" \
...
[[ -f "$LMGPT_PROJECT_DIR/empty.txt" ]] && _pass "empty file exists" || _fail "no empty file"

# Undo empty new file (before_hash=null → should delete)
trace_undo 1 force 2>/dev/null
[[ ! -f "$LMGPT_PROJECT_DIR/empty.txt" ]] && _pass "undo new empty file: deleted" || _fail "empty file still exists"
...
for i in $(seq 1 20); do
    trace_record "$LMGPT_PROJECT_DIR/hash_test.json" "v$((i-1))" "v$i" \
...
echo 'v1' > "$LMGPT_PROJECT_DIR/agent_test.txt"

# Simulate main agent edit
AGENT_SELF_NAME="main"
trace_record "$LMGPT_PROJECT_DIR/agent_test.txt" 'v1' 'v2' \
...
echo 'v2' > "$LMGPT_PROJECT_DIR/agent_test.txt"
trace_record "$LMGPT_PROJECT_DIR/agent_test.txt" 'v2' 'v3' \
...
echo 'v3' > "$LMGPT_PROJECT_DIR/agent_test.txt"
trace_record "$LMGPT_PROJECT_DIR/agent_test.txt" 'v3' 'v4' \
...
# Create 5 frames
for i in $(seq 1 5); do
    echo "v$i" > "$LMGPT_PROJECT_DIR/nest.txt"
    trace_record "$LMGPT_PROJECT_DIR/nest.txt" "v$((i-1))" "v$i" \
        "$(jq -nc --arg a "main" --arg t "edit_file" --arg d "step $i" --argjson n "$i" \
           '{agent:$a,tool:$t,turn:$n,desc:$d}')"
done

# Request more undo than frames
echo 'v5' > "$LMGPT_PROJECT_DIR/nest.txt"
...
echo 'important data' > "$LMGPT_PROJECT_DIR/delete_me.txt"
_del_content=$(cat "$LMGPT_PROJECT_DIR/delete_me.txt")
trace_record "$LMGPT_PROJECT_DIR/delete_me.txt" "$_del_content" "" \
...
_del_after=$(printf '%s' "$_del_frame" | jq -r '.changes["'"$LMGPT_PROJECT_DIR"'/delete_me.txt"].after // "NULL"')
[[ "$_del_after" == "" ]] && _pass "delete: after_hash is empty (explicit delete marker)" || _fail "delete: after_hash=$_del_after"

# Delete the actual file
rm -f "$LMGPT_PROJECT_DIR/delete_me.txt"
[[ ! -f "$LMGPT_PROJECT_DIR/delete_me.txt" ]] && _pass "delete: file removed from disk" || _fail "delete: file still exists"

# Undo should restore it
trace_undo 1 force 2>/dev/null
[[ -f "$LMGPT_PROJECT_DIR/delete_me.txt" ]] && _pass "undo delete: file restored" || _fail "undo delete: file not restored"
_restored_del=$(cat "$LMGPT_PROJECT_DIR/delete_me.txt")
...
mkdir -p "$LMGPT_PROJECT_DIR/deldir/sub"
echo 'file A' > "$LMGPT_PROJECT_DIR/deldir/a.txt"
echo 'file B' > "$LMGPT_PROJECT_DIR/deldir/sub/b.txt"
mkdir -p "$LMGPT_PROJECT_DIR/deldir/empty_sub"

# Build manifest (simulating tool_delete_file)
_dir_manifest="{}"
while IFS= read -r -d '' _df; do
    _drel="${_df#$LMGPT_PROJECT_DIR/deldir/}"
...
done < <(find "$LMGPT_PROJECT_DIR/deldir" -type f -print0 2>/dev/null || true)
...
trace_record "$LMGPT_PROJECT_DIR/deldir" "$_dir_manifest_str" "" \
...
rm -rf "$LMGPT_PROJECT_DIR/deldir"
[[ ! -d "$LMGPT_PROJECT_DIR/deldir" ]] && _pass "dir delete: directory removed" || _fail "dir delete: still exists"

# Undo should restore the directory tree
trace_undo 1 force 2>/dev/null
[[ -d "$LMGPT_PROJECT_DIR/deldir" ]] && _pass "undo dir: directory restored" || _fail "undo dir: not restored"
[[ -d "$LMGPT_PROJECT_DIR/deldir/sub" ]] && _pass "undo dir: subdirectory restored" || _fail "undo dir: sub missing"
[[ -f "$LMGPT_PROJECT_DIR/deldir/a.txt" ]] && _pass "undo dir: file a.txt restored" || _fail "undo dir: a.txt missing"
[[ -f "$LMGPT_PROJECT_DIR/deldir/sub/b.txt" ]] && _pass "undo dir: file b.txt restored" || _fail "undo dir: b.txt missing"
[[ "$(cat "$LMGPT_PROJECT_DIR/deldir/a.txt")" == "file A" ]] && _pass "undo dir: a.txt content OK" || _fail "undo dir: a.txt wrong"
[[ "$(cat "$LMGPT_PROJECT_DIR/deldir/sub/b.txt")" == "file B" ]] && _pass "undo dir: b.txt content OK" || _fail "undo dir: b.txt wrong"
...
echo 'original' > "$LMGPT_PROJECT_DIR/tamper_del.txt"
_tamper_content=$(cat "$LMGPT_PROJECT_DIR/tamper_del.txt")
trace_record "$LMGPT_PROJECT_DIR/tamper_del.txt" "$_tamper_content" "" \
...
rm -f "$LMGPT_PROJECT_DIR/tamper_del.txt"
echo 'someone recreated this' > "$LMGPT_PROJECT_DIR/tamper_del.txt"
...
[[ -f "$LMGPT_PROJECT_DIR/tamper_del.txt" ]] && _pass "delete+recreate: undo non-force works (empty after skips verify)" || _fail "delete+recreate: file missing"
...
_new_file="$LMGPT_PROJECT_DIR/new_undo_test.txt"
...
_edit_file="$LMGPT_PROJECT_DIR/edit_undo_test.txt"
echo 'line1
line2
line3' > "$_edit_file"
...
_tamper_file="$LMGPT_PROJECT_DIR/tamper_edit_test.txt"
...
_chain_file="$LMGPT_PROJECT_DIR/chain_test.txt"
...
# Create 5 frames
_orphan_file="$LMGPT_PROJECT_DIR/orphan_test.txt"
for i in $(seq 1 5); do
...
_diff_file="$LMGPT_PROJECT_DIR/diff_test.txt"
...
_prune_file="$LMGPT_PROJECT_DIR/prune_test.txt"
...
_mid_file="$LMGPT_PROJECT_DIR/mid_chain.txt"
# Frame 1
echo 'A' > "$_mid_file"
