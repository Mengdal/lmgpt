...
export LMGPT_TEST_MODE=1
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"

# Source lmgpt in test mode (skips main())
source "$PROJECT_DIR/lmgpt" 2>/dev/null || {
    echo "ERROR: cannot source lmgpt"
    exit 1
}
...
LMGPT_PROJECT_DIR="${LMGPT_PROJECT_DIR:-$PROJECT_DIR}"
LMGPT_MODEL="${LMGPT_MODEL:-claude-sonnet-4-6}"
LMGPT_API_KEY="${LMGPT_API_KEY:-sk-test}"
LMGPT_API_URL="${LMGPT_API_URL:-https://api.anthropic.com/v1/messages}"
LMGPT_AUTH_HEADER="${LMGPT_AUTH_HEADER:-x-api-key}"
LMGPT_AUTH_PREFIX="${LMGPT_AUTH_PREFIX:-}"
LMGPT_MAX_TOKENS="${LMGPT_MAX_TOKENS:-4096}"
LMGPT_THINKING_BUDGET="${LMGPT_THINKING_BUDGET:-0}"
LMGPT_PROTOCOL="${LMGPT_PROTOCOL:-anthropic}"
LMGPT_CACHE_MSG_TAIL="${LMGPT_CACHE_MSG_TAIL:-2}"
...
_hash_before="$CONTEXT_HASH"
_tf="${LMGPT_PROJECT_DIR:-.}/.lmgpt/todo.json"
if [[ -f "$_tf" ]]; then
...
MSG_COUNT=0
_bp=$(( MSG_COUNT - ${LMGPT_CACHE_MSG_TAIL:-2} - 1 ))
(( _bp < 0 )) && _bp=-1
