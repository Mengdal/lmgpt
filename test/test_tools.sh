...
_TOOLS_START=$(grep -n '^invalidate_tools_cache()' "$LMGPT" | head -1 | cut -d: -f1)
_TOOLS_END=$(grep -n '^_safe_confirm()' "$LMGPT" | head -1 | cut -d: -f1)
_TOOLS_END=$((_TOOLS_END - 1))
sed -n "${_TOOLS_START},${_TOOLS_END}p" "$LMGPT" > /tmp/lmgpt_tools_funcs.sh
...
cat > /tmp/lmgpt_tools_test.sh << 'TESTEOF'
set +e
source /tmp/lmgpt_tools_funcs.sh
...
done < <(bash /tmp/lmgpt_tools_test.sh 2>&1 || true)

rm -f /tmp/lmgpt_tools_funcs.sh /tmp/lmgpt_tools_test.sh
