#!/usr/bin/env bash
# test_code_integrity.sh — Static checks for non-portable patterns in lmgpt
...
_line_in_func() {
    ...
    local _start; _start=$(grep -n "^${_func}()" "$LMGPT" | head -1 | cut -d: -f1)
    [[ -z "$_start" ]] && return 1
    # Find the matching closing brace (first ^} after function start)
    local _end; _end=$(tail -n +$((_start + 1)) "$LMGPT" | grep -n '^}' | head -1 | cut -d: -f1)
    _end=$((_start + _end))
...
count=$(grep -c 'date +%s%3N\|date +%s%N' "$LMGPT" 2>/dev/null || echo 0)
if [[ $count -eq 1 ]]; then
    line=$(grep -n 'date +%s%3N\|date +%s%N' "$LMGPT" | head -1 | cut -d: -f1)
...
# ── sha256sum ──
echo "── sha256sum ──"
lines=$(safe_grep -n '\<sha256sum\>' "$LMGPT")
...
# ── stat -c %Y ──
echo "── stat -c %Y ──"
lines=$(safe_grep -n 'stat -c %Y' "$LMGPT")
bad=$(echo "$lines" | while IFS= read -r l; do
    ln=$(echo "$l" | cut -d: -f1)
...
# ── date -d @ ──
echo "── date -d @ ──"
lines=$(safe_grep -n 'date -d.*@' "$LMGPT")
bad=$(echo "$lines" | while IFS= read -r l; do
    ln=$(echo "$l" | cut -d: -f1)
...
# ── timeout N ──
echo "── timeout ──"
lines=$(safe_grep -n '\<timeout [0-9]' "$LMGPT")
...
# ── echo -n ──
echo "── echo -n ──"
count=$(safe_grep -c 'echo -n' "$LMGPT")
if [[ $count -eq 0 ]]; then
...
# ── head -c ──
echo "── head -c ──"
count=$(safe_grep -c 'head -c' "$LMGPT")
if [[ $count -eq 0 ]]; then
...
echo "── /proc/self/fd/ ──"
if safe_grep -q '/dev/fd/8' "$LMGPT"; then
...
if safe_grep -q 'ss.*tlnp.*netstat.*lsof' "$LMGPT"; then
...
# ── Bash version ──
echo "── bash version ──"
if safe_grep -q 'bash >= 4.1 required' "$LMGPT"; then
    _pass "version check requires 4.1+"
else
    _fail "version check 4.1+" "present" "missing"
fi

if safe_grep -q 'brew install bash' "$LMGPT"; then
...
early_line=$(grep -n 'brew install bash' "$LMGPT" | head -1 | cut -d: -f1)
first_declare=$(grep -n '^declare -A' "$LMGPT" | head -1 | cut -d: -f1)
...
if _grep_exists "^_str_display_width()" "$LMGPT"; then
...
if _grep_exists "^_str_char_display_width()" "$LMGPT"; then
...
if _grep_exists "^_update_term_width()" "$LMGPT"; then
...
if _grep_exists "trap.*_update_term_width.*WINCH" "$LMGPT"; then
...
if _grep_exists '_IN_ESC_TIMEOUT=' "$LMGPT"; then
...
_req_dw_count=$(safe_grep -A3 "^_req_disp_width()" "$LMGPT" | safe_grep -c '_str_display_width' || echo 0)
...
_in_complete_count=$(safe_grep -A15 '_in_complete()' "$LMGPT" | safe_grep -c 'readarray.*-t' || echo 0)
...
bare_read_n=$(safe_grep -n 'read.* -n [0-9]' "$LMGPT" | safe_grep -v 'LC_ALL=C\|#' || true)
...
_http_body_count=$(safe_grep -A1 '_content_length > 0' "$LMGPT" | safe_grep -c 'dd' || echo 0)
...
_oneshot_count=$(safe_grep -c 'input=\$(dd bs=1048576' "$LMGPT" || echo 0)
...
_req_ui_count=$(safe_grep -A50 '^_request_ui()' "$LMGPT" | safe_grep -c '_input_read_key' || echo 0)
...
hist_search_count=$(safe_grep -c 'TODO.*history search\|not yet implemented' "$LMGPT" || echo 0)
