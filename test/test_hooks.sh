#!/usr/bin/env bash
# test_hooks.sh — Unit tests for lmgpt hook system
...
PASS=0; FAIL=0
HOOK_TEST_DIR=$(mktemp -d "/tmp/lmgpt_hook_test.XXXXXX")
...
# Define hook functions in isolation (avoid sourcing entire lmgpt)
...
_hook_fire() {
    ...
    for _name in $_sorted_names; do
        ...
        _out_file=$(mktemp "/tmp/lmgpt_hook_out.XXXXXX")
...
load_hooks() {
    ...
    local -a _dirs=("$HOME/.lmgpt/hooks")
    [[ -d ".lmgpt/hooks" ]] && _dirs+=(".lmgpt/hooks")
...
test_hook_fire_timeout() {
    ...
    _out_file=$(mktemp "/tmp/lmgpt_hook_timeout.XXXXXX")
...
MDEOF
    ...
    mkdir -p "$HOME/.lmgpt/hooks"
    cp "$hook_file" "$HOME/.lmgpt/hooks/00_test.md"
    load_hooks || true
    HOME="$_saved_home"
