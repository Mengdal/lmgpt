#!/usr/bin/env bash
# test_utils.sh — Tests for lmgpt Section 2 utility functions
#
...
it "creates a temp file with custom prefix"
    local _f; _f=$(_mktemp_file "/tmp/lmgpt_test_custom.XXXXXX")
    assert_contains "$_f" "lmgpt_test_custom" "path should contain the custom prefix"
    rm -f "$_f"
