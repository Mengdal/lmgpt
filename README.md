...
Which means: **anywhere bash runs, Lmgpt runs.**

| Where you are | How Lmgpt runs |
|--------------|------------------|
...
Lmgpt is just a seed. But the direction it points is simple:
...
- 🔄 **Self-Evolution** — Lmgpt can read and modify its own source code. Add features through conversation: "add a --version flag" — it reads, edits, verifies, done
...
Lmgpt has only three dependencies — installable with a single package manager command on any mainstream platform:
...
```
1. Get the script   →   2. Install dependencies   →   3. ./lmgpt --install
```

`--install` automatically: creates the `~/.lmgpt/` directory tree, generates a default config file, registers bash/zsh hotkeys, and adds `lmgpt` to your PATH.
...
This command:
- Copies `lmgpt` to `~/.lmgpt/`
- Creates a `~/.local/bin/lmgpt` symlink (ensure `~/.local/bin` is in your PATH)
- Generates `~/.lmgpt/settings.json`
- Registers hotkeys: `Ctrl+G` (bash) or `Ctrl+T` (zsh) for quick access

Afterwards, type `lmgpt` from any directory to launch.
...
Homebrew-installed bash lives at `/opt/homebrew/bin/bash` (Apple Silicon) or `/usr/local/bin/bash` (Intel). Lmgpt uses `#!/usr/bin/env bash`, so as long as the Homebrew path appears before `/bin` in `/etc/paths`, the new version will be used automatically.
...
Lmgpt runs on Windows through WSL (Windows Subsystem for Linux).
...
> 💡 In WSL, `lmgpt` can directly access the Windows filesystem (`/mnt/c/`, `/mnt/d/`, etc.) and work with Windows project files.

---

#### 📱 Android (via Termux)

Termux is a terminal emulator and Linux environment for Android — no root required. Lmgpt fully supports Termux, including Android system API integration.
...
|--------------|---------|
| `coreutils` | Provides `realpath`, `nl`, and other GNU tools that Lmgpt depends on. **Required.** |
| `termux-api` | Android API bridge (optional). Enables Lmgpt to control phone hardware via Termux-API tools — see "Termux-API Integration" below. |

**Step 3: Install Lmgpt**
...
To use Lmgpt on a project stored on external storage, simply switch to the path:
...
With `termux-api` installed, Lmgpt can directly invoke Android system APIs for intelligent phone control.
...
Once installed, the following `termux-*` commands are immediately available. Lmgpt auto-detects these tools in the Termux environment — just ask in conversation:
...
After installation, edit `~/.lmgpt/settings.json` with your API details:
...
> **Compatible backends**: Lmgpt uses the Anthropic Messages API protocol. The default points to DeepSeek. You can also switch to:
...
Once installed and configured, just type `lmgpt`:
...
You're refactoring a legacy module with call chains too deep for your IDE to trace. Open a terminal and let Lmgpt handle it —

```bash
$ lmgpt
lmgpt> Where does PaymentService.process() get called in this project?
lmgpt> Map out the dependency graph of everything under src/auth/
lmgpt> Add Chinese comments to every endpoint in UserController
```
...
git diff main...feature-branch | lmgpt --oneshot
...
lmgpt> What are the top 5 errors in the nginx error log from the last hour?
lmgpt> Check /etc/nginx/nginx.conf for anything that could cause a 502
lmgpt> Compare the three configs under /etc/nginx/sites-enabled/ — find the inconsistencies
```
...
```bash
$ lmgpt
lmgpt> Write an awk one-liner to count requests per IP from access.log, sorted descending
lmgpt> My Android project is at ~/storage/shared/MyApp — find all deprecated API usage
```

With Termux-API, Lmgpt can even control your phone:

```bash
lmgpt> Take a photo and extract any text you find
lmgpt> Text "Boss" in my contacts: "Received, working on it"
lmgpt> Get my GPS coordinates and tell me the nearest subway station
```
...
```bash
$ lmgpt
lmgpt> Check this Python code for potential memory leaks
lmgpt> Explain what this regex matches: (?<=@)\w+(?=\.)
lmgpt> Convert this JSON to YAML
```
...
export LMGPT_API_URL="http://192.168.1.100:11434/api/chat"
export LMGPT_API_KEY="ollama"
...
# Config generation
cat schema.json | lmgpt --oneshot --stream
```
...
## 🔄 Self-Evolution: Let Lmgpt Write Its Own Features

This may be Lmgpt's most distinctive capability — because it is itself a plain-text bash script, and it has tools to read, edit, and execute files, **Lmgpt can modify its own source code.**
...
You say: "Add a --version flag"
         ↓
Lmgpt: Reads its own main() function to understand argument parsing
         ↓
Lmgpt: Uses the edit_file tool to insert a --version branch at the right spot
         ↓
Lmgpt: Runs ./lmgpt --version with the bash tool to verify
         ↓
Lmgpt: Confirms success, reports done ✅
```
...
Say you want Lmgpt to show token usage stats for the current session. You just say:

```
lmgpt> Add a /stats slash command that shows token usage and API call count for this session
```

Lmgpt will navigate its own source to find where slash commands are registered (`_slash_dispatch`), locate the statistics variables (`TURN_TOKENS_IN`, `TURN_TOKENS_OUT`…), and insert the new command implementation. It'll even run `bash -n` for a syntax check when done.

---

### What This Means

Lmgpt's development model is no longer "write code, submit PR, wait for merge, wait for release." Instead—

> **In your own terminal, through conversation, customize your own Lmgpt anytime.**

Want a new CLI flag? Say it. Want to add validation logic to a tool? Say it. Want a custom banner on startup? Say it. These changes live in your local Lmgpt copy — no waiting on upstream, no forking repos.

Of course, not every change suits this approach — major architectural refactors still deserve proper development and testing. But for the countless "could it do this small thing?" moments, self-evolution makes every Lmgpt user a co-author.

---

## ⚙️ Configuration

Lmgpt uses a 4-tier configuration priority (later overrides earlier):

```
Defaults  →  ~/.lmgpt/settings.json  →  Project .lmgpt/settings.json  →  Environment variables
```

| Setting | Env Variable | Default | Description |
|---------|-------------|---------|-------------|
| `api_url` | `LMGPT_API_URL` | DeepSeek Anthropic endpoint | API address |
| `api_key` | `LMGPT_API_KEY` | — | API key |
| `model` | `LMGPT_MODEL` | `deepseek-chat` | Model name |
| `max_tokens` | `LMGPT_MAX_TOKENS` | 8192 | Max output tokens |
| `thinking_budget` | `LMGPT_THINKING_BUDGET` | 16384 | Thinking budget |
| `proxy_url` | `LMGPT_PROXY_URL` | — | Proxy URL (http/socks4/socks5) |
| `context_window` | `LMGPT_CONTEXT_WINDOW` | 128000 | Context window size |
...
━━━━━━━━━━━━ Subsystems ━━━━━━━━━━━━
  ...
  │z compress│       │async sched  │       │ hot-reload│
  └──────────┘       └─────────────┘       └───────────┘
```
...
- 🐛 Submit [Issues](https://github.com/Mengdal/lmgpt/issues) to report bugs
...
**📊 LSRP `<table>` Tag** — Table rendering moved from tool dispatch to inline LSRP postprocessing. Format agent emits `<table>` + `</table>` tags around JSON config (`{"columns":[...],"align":[...],"rows":[...]}`); `_fmt_postprocess()` and `_fmt_postprocess_var()` scan for these blocks during LSRP→ANSI conversion, call `tool_table()` inline, and inject the rendered box-drawing output. `_fmt_init_stream_state()` resets `_FMT_IN_TABLE`/`_FMT_TABLE_BUF` per turn. `FORMAT_AGENT_META` reverts to `"tools":[]` — tables are now markup, not tool calls. Simpler, zero-latency: no dispatch round-trip.
...
**🧪 New Tests** — `test_bash_format.sh` Phase 4: 8 `&&` line-break tests (A01–A08). `test_box_frame.sh` Phase 8: 5 wide-terminal border consistency tests (wb1–wb5, 60 combinations + 200 random boxes). `test_and_wrap_stress.sh` reworked with `verify_and_count` for the every-`&&`-wraps rule (18 correctness tests + 500-line throughput).
...
**📚 Library Emission** — `_emit_render_lib()` and `_emit_request_ui_lib()` now embed a mode-aware `_clr()` helper function instead of hardcoded ANSI color variables (`DIM=`, `YELLOW=`, etc.), making the generated `render.sh` and `request_ui.sh` libraries respect `LMGPT_DARK_MODE` at call time.
...
- `_tm()` latency tracing controllable via `LMGPT_TIMING` env var
- `SYS_JSON_CACHE`: system prompt JSON cached until LMGPT.md/skills change
- `_count_active_todos()`: 30s TTL cache
- `load_lmgpt_md` / `_reload_skills_if_stale`: file stat every turn (throttle removed for correctness)
...
- **Lmgpt Bot** — [noreply@luomigateway@vip.qq.com](mailto:noreply@users.noreply.github.com)

---

Lmgpt proves one thing: the barrier to entry for AI coding assistants doesn't need to be Node, or Python, or a GPU, or even a container. It can be that `/bin/bash` that sits there waiting on every Linux machine since boot. From that starting point, servers, Raspberry Pis, phones, routers — all become potential AI nodes.

Maybe someday every Linux/Unix terminal will be running Lmgpt, bringing agents everywhere.